...
export async function processHeroDecks(herocode, heroAspect, percentageType) {
  ...
  if (herocode == "33001a" || herocode == "18001a") {//Cyclops or Gamora respectively
    ...
    if (percentageType == "synergy") {
      procedureCall = `CALL CalculateSynergy(${herocode}, ${heroAspect}, true)`;
    } else {
      procedureCall = `CALL CalculateSynergy(${herocode}, ${heroAspect}, false)`;
    }
    //for ordinary heroes that play nice
    // aspectDecks = [];
  ...
  connection.query(procedureCall, (error, results) => {
    if (error) {
      ...
      const cardInfo = results[0].map(row => {
        return {
      ...
      buildHeroHeader(heroName, heroAspect, cardInfo[0].totalChosenDecks, heroHeaderDiv);
